...
VulkanCommon::VulkanForge_outcome VulkanCommon::InitPipeline(VulkanForge_info& info) {
    VulkanForge_outcome outcome = {};
    ...
    outcome.vkResult = vkCreatePipelineCache(info.device, &pipelineCache, NULL, &info.pipelineCache);
    if (outcome.vkResult) return outcome;
    ...
    outcome.vkResult = vkCreateGraphicsPipelines(info.device, info.pipelineCache, 1, &pipeline, NULL, &info.pipeline);
